---
title: "teLCA Workflow"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{teLCA Workflow}
...
`teLCA` implements the BCH and ML bias-adjusted three-step estimators for
...
Bakk, Tekle & Vermunt (2013). `teLCA` also builds on top of the two-step LCA estimation procedure outlined by Bakk & Kuha (2018), and using the R package `multilevLCA` for efficient measurement model estimation from Lyrvall et al.
(2025). `teLCA` provides analytic sandwich variance estimation that propagates measurement
...
only, returning a `teLCA_measurement` object. Internally this calls
...
The standard `coef()` and `vcov()` S3 methods work on any `teLCA` object.
...
logit parameterisation. The `rebase` argument changes this. Estimates are
reparameterised consistently: log-likelihoods are invariant, and the
...
The `step1` argument accepts any previously fitted `teLCA` object or the raw
...
`teLCA` uses a two-pass row-filtering strategy that matches `multilevLCA`'s
...
`teLCA` supports polytomous indicators, following `multilevLCA`'s convention
